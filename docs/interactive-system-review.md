...
`interactive_sessions` 的 `date` 欄位是 **自由文字**（非 date 型別），學員端顯示為 `2026年{date}`。

- ✓ 正確格式：`8/20（四）` → 顯示 `2026年8/20（四）`
- ✗ 若輸入：`2026-08-20` → 顯示 `2026年2026-08-20`（格式錯誤）

新增場次時請使用 `8/20（四）` 的格式（月/日（星期）），不含年份。
