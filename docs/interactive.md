...
| `group_serial` | 集體互動序號（-1 ~ 70） |
| `small_serial` | 分組互動序號（-1 ~ 70） |
...
- **步驟**：
  ...
  4. 中簽者自動分配 `group_serial`（從現有最大序號 + 1 開始遞增）
...
- **步驟**：
  ...
  3. 意願用盡仍未中 → 沒中簽
  4. 中簽者**自動分配日期**：選該老師剩餘名額最多的日期（盡量平均）
  5. 中簽者自動分配 `small_serial`（從現有最大序號 + 1 遞增）
