...
**集體總名額**：18（中簽），候補每場次 4 位（共 12 位候補）
...
- **Phase 2（候補）**：Phase 1 結束後，仍為 lost 的學員依意願順序嘗試候補
  ...
  2. 候補名額：每個場次 **4 位**
  3. 候補者分配 `assigned_session`、`group_serial`（候補序號，per 場次從 1 開始，與中簽序號獨立）
  4. 所有候補名額也滿 → `lost`

**序號規則（集體）**：每個場次的中簽與候補各有獨立序號。例如：

| 場次 | 中簽名額 | 中簽序號 | 候補名額 | 候補序號 |
|------|---------|---------|---------|---------|
| s1（8/20 宋猜） | 5 | 1～5 | 4 | 1～4 |
| s2（8/21 奧蘭努） | 8 | 1～8 | 4 | 1～4 |
| s3（8/24 阿姜給） | 5 | 1～5 | 4 | 1～4 |
