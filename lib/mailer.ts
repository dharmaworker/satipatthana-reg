...
export async function sendMailBatch(
  ...
  const useAlicloud = process.env.ALICLOUD_THROTTLE === '1'
  const direct = useAlicloud ? mails.filter(m => !isThrottled(m.to)) : mails
  const queued = useAlicloud ? mails.filter(m => isThrottled(m.to)) : []
