...
export async function GET(request: NextRequest) {
  ...
  const { data: reg, error: regErr } = await supabaseAdmin
    .from('registrations')
    .select('id, random_code, chinese_name, passport_name, dharma_name, identity, gender, age, email, phone, member_id, student_id, status, payment_plan, payment_status')
    .eq('id', id)
      ...
      await sendMailWithRetry({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 680px; margin: 0 auto; padding: 20px; color: #222;">
            ...
            ${quickTestsButtonHtml({ id: reg.id, random_code: reg.random_code }, schedCfg)}
