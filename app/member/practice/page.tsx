...
function PracticeContent() {
  ...
  const toggle = async (item: PracticeItem) => {
    ...
    try {
      ...
      if (res.ok) {
        setItems(prev => prev.map(i => i.id === item.id ? { ...i, change_count: data.change_count ?? i.change_count } : i))
            ...
            {config.zoom_meeting_id && (
              <div style={{
                ...
                <div style={{ display: 'grid', gap: 4, fontSize: 13, color: 'var(--ink-soft)' }}>
                  ...
                  <div>登錄名：<strong style={{ color: 'var(--ink)' }}>學號 + 英文姓名／姓名拼音</strong><span style={{ color: 'var(--ink-mute)', fontSize: 12, marginLeft: 6 }}>（若無學號則用報名序號）</span></div>
                  {config.zoom_url && (
