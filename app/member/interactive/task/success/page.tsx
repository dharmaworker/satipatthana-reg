...
function TaskSuccessContent() {
  const searchParams = useSearchParams()
  const id = searchParams.get('id') || ''
  const code = searchParams.get('code') || ''
  const dashboardUrl = id && code ? `/member/dashboard?id=${id}&code=${encodeURIComponent(code)}` : '/member'
  const taskUrl = id && code ? `/member/interactive/task?id=${id}&code=${encodeURIComponent(code)}` : '/member'
    ...
    <>
      ...
      <main className="container" style={{ position: 'relative', zIndex: 1 }}>
        <div className="success-card">
          <h1 className="success-title">互動作業已送出</h1>
          <p className="success-desc">
            系統已收到您的互動作業。<br />
            如有調整請於課程開始前回到此頁修改後再次送出。
          </p>

          <div className="success-next">
            <h5>接下來</h5>
            <ol>
              <li>老師會依您填寫的問題與背景準備指導內容</li>
              <li>可於課程開始前回到此頁修改並重新送出</li>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <a href={taskUrl} className="btn btn-ghost">返回互動作業</a>
          </div>
